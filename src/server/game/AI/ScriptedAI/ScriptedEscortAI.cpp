...
void EscortAI::EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        TC_LOG_DEBUG("scripts.ai", "EscortAI::EnterEvadeMode: left combat and is now returning to last point (%s)", me->GetGUID().ToString().c_str());
    }
    else
    {
...
void EscortAI::MovementInform(uint32 type, uint32 id)
{
    ...
    if (type == POINT_MOTION_TYPE)
    {
        ...
        {
            TC_LOG_DEBUG("scripts.ai", "EscortAI::MovementInform: returned to before combat position (%s)", me->GetGUID().ToString().c_str());
            me->SetWalk(!_running);
            RemoveEscortState(STATE_ESCORT_RETURNING);
        }
        else if (id == POINT_HOME)
        {
            TC_LOG_DEBUG("scripts.ai", "EscortAI::MovementInform: returned to home location and restarting waypoint path (%s)", me->GetGUID().ToString().c_str());
            _started = false;
        }
    }
    else if (type == WAYPOINT_MOTION_TYPE)
    {
        ...
        TC_LOG_DEBUG("scripts.ai", "EscortAI::MovementInform: waypoint node %u reached (%s)", waypoint.id, me->GetGUID().ToString().c_str());
...
void EscortAI::UpdateAI(uint32 diff)
{
        ...
        if (_pauseTimer <= diff)
        {
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                if (_ended)
                {
                    ...
                    if (_despawnAtEnd)
                    {
                        TC_LOG_DEBUG("scripts.ai", "EscortAI::UpdateAI: reached end of waypoints, despawning at end (%s)", me->GetGUID().ToString().c_str());
                        if (_returnToStart)
                        {
                            ...
                            TC_LOG_DEBUG("scripts.ai", "EscortAI::UpdateAI: returning to spawn location: %s (%s)", respawnPosition.ToString().c_str(), me->GetGUID().ToString().c_str());
                        }
                    ...
                    TC_LOG_DEBUG("scripts.ai", "EscortAI::UpdateAI: reached end of waypoints (%s)", me->GetGUID().ToString().c_str());
    ...
    if (_despawnAtFar && HasEscortState(STATE_ESCORT_ESCORTING) && _playerGUID && !me->IsEngaged() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (_playerCheckTimer <= diff)
        {
            ...
            {
                TC_LOG_DEBUG("scripts.ai", "EscortAI::UpdateAI: failed because player/group was to far away or not found (%s)", me->GetGUID().ToString().c_str());
...
void EscortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, ObjectGuid playerGUID /* = 0 */, Quest const* quest /* = nullptr */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    ...
    if (me->GetVictim())
    {
        TC_LOG_ERROR("scripts.ai", "EscortAI::Start: (script: %s) attempts to Start while in combat (%s)", me->GetScriptName().c_str(), me->GetGUID().ToString().c_str());
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        TC_LOG_ERROR("scripts.ai", "EscortAI::Start: (script: %s) attempts to Start while already escorting (%s)", me->GetScriptName().c_str(), me->GetGUID().ToString().c_str());
        return;
    }
    ...
    {
        TC_LOG_ERROR("scripts.ai", "EscortAI::Start: (script: %s) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u) (%s)", me->GetScriptName().c_str(), quest ? quest->GetQuestId() : 0, me->GetGUID().ToString().c_str());
        return;
    }
    ...
    if (_returnToStart && _instantRespawn)
        TC_LOG_DEBUG("scripts.ai", "EscortAI::Start: (script: %s) is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn (%s)", me->GetScriptName().c_str(), me->GetGUID().ToString().c_str());
    ...
    TC_LOG_DEBUG("scripts.ai", "EscortAI::Start: (script: %s) started with %u waypoints. ActiveAttacker = %d, Run = %d, Player = %s (%s)",
